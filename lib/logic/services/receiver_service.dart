...
class ReceiverService extends ChangeNotifier {
  ...
  static Future<List<Receiver>> _run(String _ip) async {
    ...
    final futuresPeerQr = <Future<Receiver?>>[];

    for (final ping in futuresPing.entries) {
      final p = await ping.value;

      if (p) {
        futuresPeerQr.add(_hasPeerQr(ping.key));
      }
    }

    final result = <Receiver>[];

    for (final peerqr in futuresPeerQr) {
      final r = await peerqr;
      if (r != null) {
        result.add(r);
      }
    }
  ...
  static Future<Receiver?> _hasPeerQr(NetworkAddr addr) async {
    try {
      final result = await http
          .get(Uri.parse('http://${addr.ip}:${addr.port}/peerqr.json'))
